...
/**
 * Gestionnaire pour récupérer les membres d'un groupe d'utilisateurs CyberArk par ID
 * GET /api/cyberark/groups/[id]
 */
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const apiUrl = `${ensureTrailingSlash(baseUrl)}${CYBERARK_ENDPOINTS.GROUPS.MEMBERS.replace("{id}", id)}`;
